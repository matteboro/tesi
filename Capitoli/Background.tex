...
\begin{example}\label{ex:intervalDomain}
...
\begin{theorem}[Teorema del fix-point di Kleene]\label{teo:kleeneFixPoint}
...
Un linguaggio \(\mathbb{L}\) mette a disposizione una semantica, ovvero una descrizione formale e matematica del comportamento di un programma \(\rho\) scritto in \(\mathbb{L}\) partendo da uno stato iniziale (che può rappresentare la collezione di elementi di input di \(\rho\)). Da tale semantica si possono ottenere diverse \textit{semantiche concrete}, ovvero funzioni \(\textbf{S}:Program_\mathbb{L}\rightarrow \mathbb{C}\),  che prendono un programma \(\rho\) in \(\mathbb{L}\) e restituiscono un oggetto dell'insieme \(\mathbb{C}\), detto \textit{dominio semantico concreto}, che ci dice qualcosa sul comportamento di \(\rho\) e attraverso cui è pssibile verificarne certe proprietà. Una caratteristica necessaria di un dominio concreto è che deve essere parte di un reticolo completo, per semplicità scriveremo \(\simplePoset{\mathbb{C}}\). Due esempi di semantiche concrete sono:
\begin{itemize}
...
Per approfondire i concetti si semantica concreta, semantica stratta e inetrpretazione astratta in questa sezioni li vedremo applicati a programmi scritti in un linguaggio di programmazione, il linguaggio CFG. Un Control Flow Graph (CFG) è una rappresentazione di un programma tramite una coppia \(\cfg\) dove \(N\) è un insieme di nodi, questi rappresentano blocchi di codice basilari del programma (ad esempio gli statement), e \(E\subset N\times N\) è l'insieme di archi che collegano i nodi in \(N\), questi rappresentano il flusso di informazione tra i blocchi. Gli archi possono essere sequenziali (l'informazione passa dal nodo di partenza a quello di arrivo sempre senza modifiche), veri (l'informazione passa solo se la condizione nel nodo di partenza è vera) o falsi (opposti agli archi veri). In questo modo possiamo rappresentare anche l'if statement all'interno del linguaggio CFG. Per rappresentare i loop statement basta creare dei cicli all'interno del CFG.
...
\
\begin{definition}[Narrowing]\label{def:narrowing}
...
Nell'esempio che mostreremo ora analizzeremo il codice in \ref{fig:codiceEsempio2} cercando una approssimazione della collecting semantics tramite interpretazione astratta usando il dominio degli intervalli interi. Il sistema di equzioni astratte legate al CFG è questo:
\[
F^{\natural}=
\begin{cases}
    x_1 = \top \\
...
\begin{minipage}{\textwidth}
    ...
    \vspace{1cm}
        ...
        \]
    \end{minipage}
    \caption{Tabella dei risultati dell'analisi di fase ascendente e discendente sul programma \ref{fig:codiceEsempio2}.}
    \label{fig:ris}
\end{table}
\end{example}
...
\begin{definition}[Non ridondanza]
...
\begin{definition}[Funzione di riduzione]\label{def:funzioneRiduzione}
...
\begin{definition}[dominio delle parti dell'insieme finite e non ridondanti]\label{def:powersetDomain}
...
\begin{definition} [relazione di ordine parziale di Egli-Milner]\label{def:EgliMilnerRelation}
...
\begin{definition}[connettore di Egli-Milner]\label{def:connettoreEgliMilner}
...
\begin{definition}[ordinamento parziale indotto da widening] 
...
\begin{definition}[estrapolatore euristico]
...
\begin{definition}[estrapolatore euristico \(\nabla\)-connesso]\label{def:estrapolatoreEuristicoWideningConnesso}
...
\begin{definition}[il \(_{\textrm{EM}}\nabla_P\)widening]\label{def:wideningPowersetDomain}
