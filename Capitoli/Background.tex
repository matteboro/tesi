...
Riassumendo il concetto e riagganciandoci all'esempio \ref{ex:easyCode} nella prima fase ascendente utilizzeremo, per raggiungere un post-fix-point di \(F^{\natural}\), una versione del sistema di equazione che fa il join tra gli step della iterazione utilizzando il lub od il widening (con una logica in grado di decidere quando usare l'uno o l'altro), questo passaggio lo chiamiamo \(F^{\nabla}\). Nella fase discendente invece, partendo dal risultato calcolato da quella prima, modificheremo il sistema di equazioni in modo da fare il meet tra gli step utlizzando o il glb o il narrowing (anche dietro questa scelta ci deve essere una logica), questo passaggio lo chiamiamo \(F^{\Delta}\). 
...
\begin{tikzpicture}
    ...
    \draw [fill=white, line width=0.6mm] (0.4, 1) circle (0.15cm);
    
    \draw [fill=white, line width=0.6mm] (-0.3, 2) circle (0.15cm);

    \draw[->, blue, line width=0.5mm] (bottomStart.center) to [out=120, in=240] (widPost);

    \draw[->, red, line width=0.5mm] (widPost.center) to [out=320, in=110] (narrPost);
\end{tikzpicture}
    \centering
    \caption{Caption}
    \label{fig:my_label}
